...
if not TELEGRAM_BOT_TOKEN or not BOT_USERNAME or not WEBHOOK_URL:
    logging.error("CRITICAL: TELEGRAM_BOT_TOKEN, BOT_USERNAME, and WEBHOOK_URL must be set in env.")
    sys.exit(1)
...
async def cancel_giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    user_id = update.effective_user.id
    GIVEAWAY_CREATION_DATA.pop(user_id, None)
    await update.message.reply_text("🛑 Giveaway creation cancelled.", parse_mode=ParseMode.MARKDOWN)
    return ConversationHandler.END

# Participant flow (Deep Link)
async def handle_deep_link_participation(update: Update, context: ContextTypes.DEFAULT_TYPE, giveaway_id: str) -> None:
    ...
    if not is_member:
        caption_text = (
            ...
            f"To join <b>'{channel_name}'</b> poll you must be a subscriber. Join then try again."
        )
    ...
    if success:
        ...
        try:
            ...
            await update.effective_message.reply_text(
                f"🎉 You are registered for '{channel_name}' poll (ID: <code>{giveaway_id}</code>).",
                parse_mode=ParseMode.HTML
            )
        except Exception as e:
            logger.error(f"Failed to post participant to channel {channel_id}: {e}")
            await update.effective_message.reply_text("Participation logged, but failed to post to channel. Check bot's permissions.")
    else:
        await update.effective_message.reply_text("💡 You have already participated in this poll.", parse_mode=ParseMode.MARKDOWN)

# Admin features: top list, active polls, closing poll
async def show_top_participants(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.message and query.message.chat_id < 0 and not is_admin(query.from_user.id):
        await query.answer("You can only view this list in private chat.", show_alert=True)
        return
    await query.answer("Fetching top participants...")
    try:
    ...
    if not data:
        ...
        for i, (full_name, username, user_id, participation_time) in enumerate(data):
            try:
...
async def active_polls(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not active:
        await update.effective_message.reply_text("No active polls.", parse_mode=ParseMode.MARKDOWN)
        return
    lines = ["✨ <b>ACTIVE POLLS:</b>\n"]
    for i, (gid, channel_id, start_time) in enumerate(active):
        try:
            ch = await context.bot.get_chat(channel_id)
            channel_name = ch.title
        except TelegramError:
            channel_name = f"ID: {channel_id}"
        lines.append(f"<b>{i+1}.</b> {channel_name}\n   ID: <code>{gid}</code>\n   Start: <i>{start_time.split('.')[0]}</i>\n   Close: <code>/close_poll_{gid}</code>\n")
    await update.effective_message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)
...
async def start_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    if not context.args:
        await update.message.reply_text("Usage: /broadcast <GIVEAWAY_ID>")
        return ConversationHandler.END
    giveaway_id = context.args[0]
    ...
    await update.message.reply_text("Send the message (text/photo/video/animation) to broadcast to participants.")
    ...
    for uid in participants:
        try:
            if getattr(update.message, "photo", None):
                await context.bot.send_photo(chat_id=uid, photo=update.message.photo[-1].file_id, caption=caption_payload or "", parse_mode=ParseMode.HTML)
            elif getattr(update.message, "video", None):
                await context.bot.send_video(chat_id=uid, video=update.message.video.file_id, caption=caption_payload or "", parse_mode=ParseMode.HTML)
            elif getattr(update.message, "animation", None):
                await context.bot.send_animation(chat_id=uid, animation=update.message.animation.file_id, caption=caption_payload or "", parse_mode=ParseMode.HTML)
            elif text_payload:
...
def main() -> None:
    ...
    logger.info(f"Starting webhook on port {PORT} at /{url_path}")
